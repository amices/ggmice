...
You may want to create a plot visualizing the imputations of multiple variables as one object. This can be done by combining `ggmice` with `purrr` and `patchwork`. For example, we could obtain boxplots of different imputed variables as one object.

```{r facet}
purrr::map(list("wgt", "hgt", "bmi"), function(x){
  # ggmice equivalent
