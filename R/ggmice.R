...
ggmice <- function(data = NULL,
  ...
  if (is.character(mapping$x) | is.character(mapping$y)) {
    stop(
      "The mapping argument requires variable name(s) of type 'quosure', typically created with ggplot2::aes(). To supply a string instead, try using ggplot2::aes_string()"
    )
  }
  if (!is.null(mapping$colour)) {
    warning(
      "The aes() argument 'colour' has a special use in ggmmice() and will be overwritten. Try using 'shape' or 'linetype' for additional mapping, or use faceting."
    )
  }
  ...
  if (!is.null(mapping$x) &
      mapping_x %nin% c(".id", ".imp", ".where") &
      identical(vrb_x, character(0))) {
    stop(paste0(
      "Mapping variable '",
      mapping_x,
  ...
  if (!is.null(mapping$y) &
      mapping_y %nin% c(".id", ".imp", ".where") &
      identical(vrb_y, character(0))) {
    stop(paste0(
      "Mapping variable '",
      mapping_y,
      "' not found in the data or imputations."
